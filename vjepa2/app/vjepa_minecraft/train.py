...
#
# --- VPT Edit ---
# This script has been adapted from the DROID V-JEPA 2-AC training script
# to support the WebDataset-based VPT (Minecraft) data loader.
#
...
def main(args, resume_preempt=False):
    ...
    use_extrinsics = cfgs_model.get("use_extrinsics", False) # Must be False for VPT
    
    # --- VPT Edit ---
    # Load action_embed_dim from config.
    # This MUST match the output dimension of vpt_states_to_diffs()
    # Example: 1(dx) + 1(dy) + 1(dyaw) + 1(dpitch) + 9(keys) = 13
    action_embed_dim = cfgs_model.get("action_embed_dim")
    if action_embed_dim is None:
        raise ValueError("Config ERROR: model.action_embed_dim must be specified.")
    logger.info(f"Using action_embed_dim: {action_embed_dim}")
    if use_extrinsics:
        logger.warning("Config WARNING: use_extrinsics is True, but VPT data does not provide extrinsics.")
    # --- End VPT Edit ---
    ...
    # We still read them so the config format is the same, but they are ignored.
    _ = cfgs_data.get("camera_frame", False)
    _ = cfgs_data.get("camera_views", ["left_mp4_path"])
    _ = cfgs_data.get("stereo_view", False)
    # --- End VPT Edit ---
    
    batch_size = cfgs_data.get("batch_size")
    tubelet_size = cfgs_data.get("tubelet_size") # This will be passed as `frameskip`
    ...
    encoder, predictor = init_video_model(
        ...
        max_num_frames=512, # This is max *model* context, not data context
        ...
        # --- VPT Edit ---
        action_embed_dim=action_embed_dim, # Pass the config value
        # --- End VPT Edit ---
        pred_is_frame_causal=pred_is_frame_causal,
        use_extrinsics=use_extrinsics, # This will be False
        use_sdpa=use_sdpa,
        use_silu=use_silu,
    ...
    # --- VPT Edit ---
    # The DROID loader required a separate transform and collator.
    # Our WebDataset loader handles transforms internally, so we don't need these.
    # We leave `transform` creation for now, as it doesn't hurt.
    # We remove `video_collator`.
    # --- End VPT Edit ---
    ...
    # --- VPT Edit ---
    # We do not have a sampler object to set the epoch on.
    # WebDataset's `.repeat()` and `.split_by_node` handles this.
    if unsupervised_sampler is not None:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # --- VPT Edit ---
        # Set epoch for sampler if it exists (it shouldn't for us)
        if unsupervised_sampler is not None:
            unsupervised_sampler.set_epoch(epoch)
        # --- End VPT Edit ---
        ...
        for itr in range(ipe):
            ...
            # --- VPT Edit ---
            # Rewrite load_clips to use our sample dictionary
            def load_clips():
            ...
            def train_step():
                ...
                def forward_target(c):
                    with torch.no_grad():
                        # --- VPT Edit ---
                        # Our clips are already (B, C, T, H, W) from load_clips()
                        # The original permute was for (B, T, C, H, W)
                        # We adjust to match the original logic's input to encoder
                        # Original: (B C T H W) -> permute(0, 2, 1, 3, 4) -> (B T C H W)
                        # Our clips: (B, C, T, H, W)
                        # We need to permute to (B, T, C, H, W)
                ...
                def forward_predictions(z):
                    ...
                    def _step_predictor(_z, _a, _s, _e):
                        # --- VPT Edit ---
                        # _e (extrinsics) will be None.
                        # The predictor model (from init_video_model)
                        # must handle this based on use_extrinsics=False.
                        # --- End VPT Edit ---
                    ...
                    # --- VPT Edit ---
                    # states is (B, T_states, D_states), T_states = T_frames / frameskip
                    # actions is (B, T_actions, D_actions), T_actions = T_states - 1
                    # extrinsics is None
                    # We need states[:, :-1] to align with actions
                    _z, _a, _s, _e = z[:, :-tokens_per_frame], actions, states[:, :-1], extrinsics 
                    # --- End VPT Edit ---
                    ...
                    for n in range(1, auto_steps):
                        # --- VPT Edit ---
                        # _e will be None, which is fine
                        _a, _s, _e = actions[:, : n + 1], states[:, : n + 1], extrinsics 
                        # --- End VPT Edit ---
...
# We'll assume it's called from another script that parses configs,
# so we don't add an `if __name__ == "__main__":` block.
